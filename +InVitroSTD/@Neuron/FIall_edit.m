function FIall_edit(self)
%calculate FI curve for all Recordings in a cell/path using 1/(t(2)-t(1))
        ...
        for j = 1:length(timeInd)
            if ~vec(j)
                ...
                if length(timeInd{j}) == 1
                    ...
                    outs(count).freq(ind) = 1 / (timeInd{j}(end) / Fs - timeInd{j}(end-1) / Fs);
                end
                ind = ind + 1;
            end
        end
