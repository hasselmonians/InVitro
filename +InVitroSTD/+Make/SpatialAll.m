function [sig ts sig_normal sig_normalized sig_halfMag sig_random] = SpatialAll(sinFreq,excitInhib,ifSave)
%{
matlabpool open
freqs = 2:1:12;
parfor i=1:length(freqs)
    make.SpatialAll(freqs(i),0,1);
    make.SpatialAll(freqs(i),1,1);
end
%}
%% File parameters
sampFreq = 5000;%5000;
...
folderName = ['/media/RatBrains/software/Library/SpatialAll/' ];
if excitInhib == 0
...
sig_normal = make.superImposed(sinSig,doubleExp_sig,[],0,16,0);
sig_normalized = make.superImposed(sinSig,doubleExp_sig,[],0,16,var);
sig_halfMag = make.superImposed(sinSig,0.5*doubleExp_sig,[],0,16,0);
sig_random = make.superImposed(sinSig,2*doubleExp_sig,[],1,16,0);
